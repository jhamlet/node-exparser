...
module.exports = Parser = PClass.derive({
    ...
    init: function (opts) {
        ...
        if (opts) {
            self = (exps = opts.expression) && this.addExpressions(exps);
     ...
     */
    parse: function (src) {
        var scanner, startPatterns, matchLen;
        ...
        startPatterns = this.getStartPatternUnion();
        while (scanner.scanUntil(startPatterns)) {
     ...
     * expression sequence
     * 
     * @method getStartPatternUnion
     * @returns {RegExp}
     */
    getStartPatternUnion: function () {
        var map = {};
