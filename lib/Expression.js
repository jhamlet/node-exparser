...
module.exports = Expression = Proteus.Class.derive({
    ...
    /**
     * Used to determine the start pattern for a function passed as a pattern
     * 
    ...
    exec: function (txt) {
        ...
        if (results) {
            this.match = this.scanner.source.slice(this.startPos, this.scanner.getPosition());
    ...
    getStartPattern: function () {
        ...
        pat = pat.getStartPattern ? pat.getStartPattern() : pat;
